...
.message{min-height:22px;margin:12px 4px 0;color:#777;font-size:13px}.message.error{color:#eee}
...
.playerCard{display:none;margin-top:26px}.playerCard.show{display:block;animation:rise .32s ease both}.playerShell{position:relative;width:100%;aspect-ratio:16/9;border-radius:26px;overflow:hidden;background:#000;border:1px solid #242424;box-shadow:0 20px 70px rgba(0,0,0,.35);touch-action:manipulation}
.playerShell video{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#000}
.playerShade{position:absolute;inset:0;background:linear-gradient(to bottom,transparent 42%,rgba(0,0,0,.78));pointer-events:none;opacity:.95}
.playerCenter{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:74px;height:74px;border-radius:50%;display:grid;place-items:center;background:rgba(8,8,8,.78);color:#fff;border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(16px);box-shadow:0 14px 40px rgba(0,0,0,.35);transition:opacity .2s,transform .2s}.playerCenter:active{transform:translate(-50%,-50%) scale(.94)}.playerCenter svg{width:34px;height:34px;stroke-width:2.8}.playerShell.playing .playerCenter{opacity:0;pointer-events:none}
.playerLoading{position:absolute;left:50%;top:50%;width:46px;height:46px;margin:-23px;border:3px solid rgba(255,255,255,.14);border-top-color:#fff;border-radius:50%;animation:spin .8s linear infinite;opacity:0;pointer-events:none}.playerShell.loading .playerLoading{opacity:1}.playerShell.loading .playerCenter{opacity:0}
.playerControls{position:absolute;left:0;right:0;bottom:0;padding:36px 16px 15px;opacity:1;transform:translateY(0);transition:opacity .22s,transform .22s;background:linear-gradient(to top,rgba(0,0,0,.88),rgba(0,0,0,.32),transparent)}.playerShell.controlsHidden .playerControls{opacity:0;transform:translateY(8px);pointer-events:none}
.seekWrap{position:relative;height:20px;display:flex;align-items:center}.seekBase{position:absolute;left:2px;right:2px;height:5px;border-radius:99px;background:rgba(255,255,255,.17);overflow:hidden}.bufferedBar,.playedBar{position:absolute;inset:0 auto 0 0;width:0;border-radius:99px}.bufferedBar{background:rgba(255,255,255,.24)}.playedBar{background:#fff}
.seek{position:relative;width:100%;height:20px;margin:0;appearance:none;-webkit-appearance:none;background:transparent;cursor:pointer}.seek::-webkit-slider-runnable-track{height:5px;background:transparent}.seek::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:#fff;margin-top:-5.5px;box-shadow:0 1px 8px rgba(0,0,0,.45)}.seek::-moz-range-track{height:5px;background:transparent}.seek::-moz-range-thumb{width:16px;height:16px;border:0;border-radius:50%;background:#fff}
.controlRow{display:flex;align-items:center;gap:6px;margin-top:6px}.controlButton{width:40px;height:40px;border-radius:12px;display:grid;place-items:center;background:transparent;color:#fff;transition:background .18s,transform .18s}.controlButton:active{transform:scale(.91);background:rgba(255,255,255,.1)}.controlButton svg{width:22px;height:22px;stroke-width:2.8}.skipButton svg{width:24px;height:24px}.playerTime{font-size:11px;font-variant-numeric:tabular-nums;color:#d8d8d8;margin-left:3px;white-space:nowrap}.controlSpacer{flex:1}
.watchMeta{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding:15px 4px 0}.watchTitle{font-size:14px;font-weight:650;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.watchBadge{flex:0 0 auto;font-size:10px;font-weight:800;letter-spacing:.08em;color:#080808;background:#eee;border-radius:99px;padding:6px 8px}
.playerShell.telegramFullscreen{position:fixed;z-index:9999;inset:0;width:100vw;height:100vh;aspect-ratio:auto;border:0;border-radius:0;padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom)}
.playerShell:fullscreen{border-radius:0;border:0}.playerShell:fullscreen .playerControls{padding-bottom:max(18px,env(safe-area-inset-bottom))}
.foot{margin-top:36px;padding:0 4px;color:#444;font-size:11px;display:flex;justify-content:space-between;gap:12px}
...
@media(max-width:420px){.app{padding-left:14px;padding-right:14px}.top{margin-bottom:36px}.head,.section{padding-left:18px;padding-right:18px}.playerControls{padding-left:10px;padding-right:10px}.controlButton{width:37px;height:38px}.playerTime{font-size:10px}}
@media(prefers-reduced-motion:reduce){*,*:before,*:after{animation:none!important;transition:none!important}}
</style>
</head>
<body>
<main class="app">
<header class="top"><div class="brand"><span class="mark">V</span>VEXA</div><div class="live"><span class="dot"></span>Downloader</div></header>
...
<section id="playerCard" class="playerCard" aria-live="polite">
  ...
  <div class="watchMeta"><div id="watchTitle" class="watchTitle"></div><span class="watchBadge">LIVE STREAM</span></div>
</section>
...
<footer class="foot"><span id="footLeft">Direct streaming</span><span id="footRight">No Bot API 50 MB limit</span></footer>
</main>
...
var modeSwitch=$('modeSwitch'),modeDownload=$('modeDownload'),modeWatch=$('modeWatch'),sub=$('sub'),footLeft=$('footLeft'),footRight=$('footRight');
...
var sessionId=(crypto.randomUUID?crypto.randomUUID().replace(/-/g,''):String(Date.now())+Math.random().toString(36).slice(2)),current=null,selected=null,prepared=null,pollToken=0,mode='download',controlsTimer=null,telegramFullscreenRequested=false;
...
function iconPause(){return '<path d="M9 7v10M15 7v10" stroke="currentColor" stroke-width="3" stroke-linecap="round"/>'}
...
function resetPlayer(){if(controlsTimer)clearTimeout(controlsTimer);video.pause();video.removeAttribute('src');video.load();playerCard.classList.remove('show');playerShell.classList.remove('playing','loading','controlsHidden','telegramFullscreen');telegramFullscreenRequested=false;seek.value='0';playedBar.style.width='0%';bufferedBar.style.width='0%';playerTime.textContent='0:00 / 0:00'}
...
async function toggleFullscreen(){showControls();if(document.fullscreenElement&&document.exitFullscreen){try{await document.exitFullscreen();return}catch(e){}}if(playerShell.requestFullscreen){try{await playerShell.requestFullscreen();return}catch(e){}}if(tg&&typeof tg.requestFullscreen==='function'){telegramFullscreenRequested=true;playerShell.classList.add('telegramFullscreen');try{tg.requestFullscreen();return}catch(e){playerShell.classList.remove('telegramFullscreen')}}if(video.webkitEnterFullscreen){try{video.webkitEnterFullscreen()}catch(e){}}}
function syncMuteIcon(){muteIcon.innerHTML=video.muted?'<path d="M5 10v4h4l4 4V6L9 10H5Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/><path d="m17 9 4 6m0-6-4 6" stroke="currentColor" stroke-width="2.8" stroke-linecap="round"/>':'<path d="M5 10v4h4l4 4V6L9 10H5Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 9.2a4 4 0 0 1 0 5.6" stroke="currentColor" stroke-linecap="round"/>'}
...
document.addEventListener('fullscreenchange',function(){if(!document.fullscreenElement)playerShell.classList.remove('telegramFullscreen');showControls()});
if(tg){try{tg.ready();tg.expand();tg.setHeaderColor&&tg.setHeaderColor('#050505');tg.setBackgroundColor&&tg.setBackgroundColor('#050505');tg.setBottomBarColor&&tg.setBottomBarColor('#050505');tg.onEvent&&tg.onEvent('fileDownloadRequested',function(e){if(e&&e.status==='downloading')msg('Download started.');else if(e&&e.status==='cancelled')msg('Download was cancelled.',true)});tg.onEvent&&tg.onEvent('fullscreenChanged',function(){var on=!!tg.isFullscreen&&telegramFullscreenRequested;playerShell.classList.toggle('telegramFullscreen',on);if(!tg.isFullscreen)telegramFullscreenRequested=false;showControls()});tg.onEvent&&tg.onEvent('fullscreenFailed',function(){playerShell.classList.remove('telegramFullscreen');telegramFullscreenRequested=false;showControls()})}catch(e){}}else msg('Open this page from the Vexa bot in Telegram.',true);

...
async function signDownload(botToken: string, payload: string): Promise<string> { return bytesToHex(await hmacSha256(encoder.encode(botToken), payload)); }
function signaturePayload(userId: number, sessionId: string, fileId: string, expires: number): string { return `${userId}|${sessionId}|${fileId}|${expires}`; }
function makeDownloadUrl(request: Request, userId: number, sessionId: string, fileId: string, expires: number, sig: string): string {
  ...
  url.searchParams.set("u", String(userId)); url.searchParams.set("s", sessionId); url.searchParams.set("f", fileId); url.searchParams.set("e", String(expires)); url.searchParams.set("sig", sig);
  return url.toString();
}
...
async function handleStatus(request: Request, env: MiniAppEnv): Promise<Response> {
  ...
  const sig = await signDownload(env.BOT_TOKEN, signaturePayload(auth.identity.userId, auth.sessionId, result.fileId, expires));
...
async function handleFile(request: Request, env: MiniAppEnv): Promise<Response> {
  const url = new URL(request.url); const userId = Number(url.searchParams.get("u") || "0"); const sessionId = url.searchParams.get("s") || ""; const fileId = url.searchParams.get("f") || ""; const expires = Number(url.searchParams.get("e") || "0"); const sig = url.searchParams.get("sig") || ""; const now = Math.floor(Date.now() / 1000);
  ...
  const expected = await signDownload(env.BOT_TOKEN, signaturePayload(userId, sessionId, fileId, expires)); if (!constantTimeEqual(expected.toLowerCase(), sig.toLowerCase())) return new Response("Download link expired or invalid.", { status: 403 });
  const container = getContainer(env.YOUTUBE_DOWNLOADER_CONTAINER, miniContainerId(userId, sessionId)); const headers = new Headers(); const range = request.headers.get("range"); if (range) headers.set("range", range);
  ...
  const outgoing = new Headers(); for (const name of ["content-type", "content-length", "content-range", "accept-ranges", "content-disposition"]) { const value = source.headers.get(name); if (value) outgoing.set(name, value); }
  outgoing.set("cache-control", "private, no-store"); outgoing.set("access-control-allow-origin", "https://web.telegram.org"); outgoing.set("cross-origin-resource-policy", "cross-origin"); outgoing.set("x-content-type-options", "nosniff");
  return new Response(request.method === "HEAD" ? null : source.body, { status: source.status, headers: outgoing });
}

const HTML = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover,user-scalable=no"><meta name="theme-color" content="#050505"><title>Vexa Downloader</title><script src="https://telegram.org/js/telegram-web-app.js?63"></script><style>:root{color-scheme:dark;--bg:#050505;--text:#f5f5f5;--muted:#888;--line:#222}*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}html,body{margin:0;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","SF Pro Text",Inter,system-ui,sans-serif;min-height:100%;overflow-x:hidden}body:before{content:"";position:fixed;inset:-25%;pointer-events:none;background:radial-gradient(circle at 50% 8%,rgba(255,255,255,.07),transparent 28%);animation:drift 10s ease-in-out infinite alternate}.app{position:relative;max-width:720px;margin:0 auto;padding:max(24px,env(safe-area-inset-top)) 18px max(30px,env(safe-area-inset-bottom));min-height:100vh}.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:52px;animation:rise .55s ease both}.brand{display:flex;align-items:center;gap:10px;font-size:14px;font-weight:700;letter-spacing:.22em}.mark{width:28px;height:28px;border:1px solid #292929;border-radius:50%;display:grid;place-items:center}.live{color:#777;font-size:12px;display:flex;gap:7px;align-items:center}.dot{width:6px;height:6px;border-radius:50%;background:#eee;animation:pulse 2s ease infinite}h1{font-size:clamp(38px,10vw,64px);line-height:.94;letter-spacing:-.055em;margin:0 0 18px;font-weight:650;animation:rise .6s .05s ease both}.sub{color:var(--muted);font-size:15px;line-height:1.6;margin:0 0 32px;max-width:520px}.inputShell{display:flex;align-items:center;gap:10px;padding:8px;border:1px solid var(--line);background:#0a0a0a;border-radius:22px}.linkIcon{width:42px;height:42px;border-radius:15px;background:#111;display:grid;place-items:center;color:#aaa;flex:0 0 auto}input{min-width:0;flex:1;background:transparent;color:#fff;border:0;outline:0;font:inherit;font-size:15px;padding:12px 0}input::placeholder{color:#555}button{font:inherit;border:0;cursor:pointer}.go{height:46px;border-radius:16px;padding:0 18px;background:#fff;color:#050505;font-weight:650}button:disabled{opacity:.35}.message{min-height:22px;margin:12px 4px 0;color:#777;font-size:13px}.message.error{color:#eee}.card{margin-top:26px;border:1px solid var(--line);border-radius:26px;background:#0a0a0a;overflow:hidden;opacity:0;transform:translateY(12px);pointer-events:none;transition:.3s}.card.show{opacity:1;transform:none;pointer-events:auto}.head{padding:22px;border-bottom:1px solid var(--line)}.eyebrow{color:#666;font-size:11px;letter-spacing:.14em;margin-bottom:10px}.title{font-size:18px;line-height:1.35;font-weight:580}.section{padding:20px 22px}.label{font-size:12px;color:#777;margin-bottom:12px}.formats{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.format{text-align:left;padding:15px;border:1px solid #222;border-radius:17px;background:#0d0d0d;color:#ddd}.format .name{display:block;font-size:14px;font-weight:600}.format .desc{display:block;font-size:11px;color:#666;margin-top:4px}.format.selected{background:#f3f3f3;color:#090909;border-color:#eee}.format.selected .desc{color:#555}.primary,.save{width:100%;height:54px;border-radius:18px;background:#fff;color:#050505;font-weight:680;margin-top:18px}.progress,.ready{display:none;margin-top:26px;border:1px solid var(--line);border-radius:26px;padding:22px;background:#090909}.progress.show,.ready.show{display:block;animation:rise .3s ease both}.progressTop{display:flex;align-items:center;gap:15px}.orb{position:relative;width:42px;height:42px;border-radius:50%;border:1px solid #262626;flex:0 0 auto}.orb:before{content:"";position:absolute;inset:5px;border-radius:50%;border:2px solid transparent;border-top-color:#fff;animation:spin 1s linear infinite}.progressTitle,.readyTitle{font-size:15px;font-weight:600}.progressSub,.readyMeta{font-size:12px;color:#666;margin-top:5px}.rail{height:2px;background:#171717;margin-top:20px;overflow:hidden}.rail:after{content:"";display:block;width:35%;height:100%;background:#eee;animation:scan 1.3s ease-in-out infinite}.readyRow{display:flex;align-items:center;gap:14px}.readyIcon{width:44px;height:44px;border-radius:50%;background:#fff;color:#050505;display:grid;place-items:center;font-size:20px}.readyText{min-width:0;flex:1}.readyMeta{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.foot{margin-top:36px;padding:0 4px;color:#444;font-size:11px;display:flex;justify-content:space-between;gap:12px}@keyframes rise{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{50%{opacity:.35}}@keyframes scan{0%{transform:translateX(-120%)}100%{transform:translateX(390%)}}@keyframes drift{to{transform:translateY(-2%) scale(1.03)}}@media(max-width:420px){.app{padding-left:14px;padding-right:14px}.top{margin-bottom:44px}.head,.section{padding-left:18px;padding-right:18px}}</style></head><body><main class="app"><header class="top"><div class="brand"><span class="mark">V</span>VEXA</div><div class="live"><span class="dot"></span>Downloader</div></header><h1>Paste.<br>Pick. Save.</h1><p class="sub">Drop a YouTube link, choose the quality you want, then save the file directly to your device.</p><div class="inputShell"><div class="linkIcon">↗</div><input id="url" type="url" inputmode="url" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="Paste YouTube link"><button id="analyze" class="go" type="button">Analyze</button></div><div id="message" class="message" aria-live="polite"></div><section id="result" class="card"><div class="head"><div class="eyebrow">YOUTUBE</div><div id="videoTitle" class="title"></div></div><div class="section"><div class="label">Choose format</div><div id="formats" class="formats"></div><button id="prepare" class="primary" type="button" disabled>Prepare download</button></div></section><section id="progress" class="progress"><div class="progressTop"><div class="orb"></div><div><div class="progressTitle">Preparing your file</div><div id="progressSub" class="progressSub">Starting download…</div></div></div><div class="rail"></div></section><section id="ready" class="ready"><div class="readyRow"><div class="readyIcon">↓</div><div class="readyText"><div class="readyTitle">Ready to save</div><div id="readyMeta" class="readyMeta"></div></div></div><button id="save" class="save" type="button">Save to Files</button></section><footer class="foot"><span>Direct streaming</span><span>No Bot API 50 MB limit</span></footer></main><script>(function(){var tg=window.Telegram&&window.Telegram.WebApp?window.Telegram.WebApp:null,$=function(i){return document.getElementById(i)},url=$('url'),analyze=$('analyze'),message=$('message'),result=$('result'),title=$('videoTitle'),formats=$('formats'),prepare=$('prepare'),progress=$('progress'),progressSub=$('progressSub'),ready=$('ready'),readyMeta=$('readyMeta'),save=$('save');var sessionId=(crypto.randomUUID?crypto.randomUUID().replace(/-/g,''):String(Date.now())+Math.random().toString(36).slice(2)),current=null,selected=null,prepared=null,pollToken=0;function initData(){return tg&&tg.initData?tg.initData:''}function msg(t,e){message.textContent=t||'';message.classList.toggle('error',!!e)}function wait(ms){return new Promise(function(r){setTimeout(r,ms)})}function size(b){if(!b)return'File ready';var u=['B','KB','MB','GB'],i=Math.min(Math.floor(Math.log(b)/Math.log(1024)),3),v=b/Math.pow(1024,i);return(v>=100||i===0?Math.round(v):v.toFixed(1))+' '+u[i]}function hap(k){try{if(tg&&tg.HapticFeedback){if(k==='success'||k==='error')tg.HapticFeedback.notificationOccurred(k);else tg.HapticFeedback.selectionChanged()}}catch(e){}}function choose(v,b){selected=v;formats.querySelectorAll('.format').forEach(function(x){x.classList.remove('selected')});b.classList.add('selected');prepare.disabled=false;prepared=null;ready.classList.remove('show');hap('select')}function add(n,d,v){var b=document.createElement('button');b.type='button';b.className='format';b.innerHTML='<span class="name"></span><span class="desc"></span>';b.querySelector('.name').textContent=n;b.querySelector('.desc').textContent=d;b.addEventListener('click',function(){choose(v,b)});formats.appendChild(b);return b}function render(d){formats.textContent='';selected=null;var pref=null;(d.qualities||[]).forEach(function(q){var b=add(q+'p',q===360?'Small file':q===480?'Balanced':q===720?'Recommended':'Full HD',{quality:q});if(q===720)pref=b});if(d.audioAvailable){add('Audio Lite','Smaller M4A',{audioMode:'low'});add('Audio HQ','Best M4A',{audioMode:'hq'})}if(!pref)pref=formats.querySelector('.format');if(pref)pref.click()}async function api(p,b){var r=await fetch(p,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(b)}),d=await r.json().catch(function(){return{ok:false,message:'Unexpected server response.'}});if(!r.ok)d.ok=false;return d}async function analyzeLink(){var v=url.value.trim();if(!v)return msg('Paste a YouTube link first.',true);if(!initData())return msg('Open this page from the Vexa bot in Telegram.',true);pollToken++;analyze.disabled=true;result.classList.remove('show');progress.classList.remove('show');ready.classList.remove('show');msg('Reading video…');try{var d=await api('/mini-app/api/metadata',{initData:initData(),sessionId:sessionId,url:v});if(!d.ok)throw new Error(d.message||'Could not read this video.');current=d;title.textContent=d.title||'YouTube video';render(d);result.classList.add('show');msg('Choose a format below.');hap('success')}catch(e){msg(e.message||'Could not read this video.',true);hap('error')}finally{analyze.disabled=false}}async function poll(jobId,token){var started=Date.now();while(token===pollToken){await wait(1500);var d=await api('/mini-app/api/status',{initData:initData(),sessionId:sessionId,jobId:jobId});if(token!==pollToken)return;if(d.ok&&d.state==='ready'){prepared=d;progress.classList.remove('show');readyMeta.textContent=size(d.size)+' · '+d.fileName;ready.classList.add('show');msg('Your file is ready.');prepare.disabled=false;analyze.disabled=false;hap('success');return}if(!d.ok||d.state==='error')throw new Error(d.message||'Could not prepare this file.');var sec=Math.round((Date.now()-started)/1000);progressSub.textContent=sec>45?'Still preparing — large videos can take a few minutes. Keep this Mini App open.':'Downloading and packaging the selected quality…';if(sec>1800)throw new Error('This download took too long. Please try again.')}}async function prepareFile(){if(!current||!selected)return;var token=++pollToken;prepare.disabled=true;analyze.disabled=true;ready.classList.remove('show');progress.classList.add('show');progressSub.textContent='Starting download…';msg('');try{var body={initData:initData(),sessionId:sessionId,videoId:current.videoId};if(selected.quality)body.quality=selected.quality;else body.audioMode=selected.audioMode;var d=await api('/mini-app/api/start',body);if(!d.ok||!d.jobId)throw new Error(d.message||'Could not start this download.');await poll(d.jobId,token)}catch(e){if(token===pollToken){progress.classList.remove('show');prepare.disabled=false;analyze.disabled=false;msg(e.message||'Could not prepare this file.',true);hap('error')}}}function saveFile(){if(!prepared||!prepared.downloadUrl)return;if(tg&&typeof tg.downloadFile==='function'){tg.downloadFile({url:prepared.downloadUrl,file_name:prepared.fileName},function(ok){msg(ok?'Download started.':'Telegram did not start the download. Tap Save again.',!ok)});return}if(tg&&typeof tg.openLink==='function'){tg.openLink(prepared.downloadUrl);return}location.href=prepared.downloadUrl}analyze.addEventListener('click',analyzeLink);prepare.addEventListener('click',prepareFile);save.addEventListener('click',saveFile);url.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();analyzeLink()}});url.addEventListener('input',function(){pollToken++;prepared=null;ready.classList.remove('show')});if(tg){try{tg.ready();tg.expand();tg.setHeaderColor&&tg.setHeaderColor('#050505');tg.setBackgroundColor&&tg.setBackgroundColor('#050505');tg.setBottomBarColor&&tg.setBottomBarColor('#050505');tg.onEvent&&tg.onEvent('fileDownloadRequested',function(e){if(e&&e.status==='downloading')msg('Download started.');else if(e&&e.status==='cancelled')msg('Download was cancelled.',true)})}catch(e){}}else msg('Open this page from the Vexa bot in Telegram.',true)})();</script></body></html>`;

function htmlResponse(): Response { return new Response(HTML, { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store", "x-content-type-options": "nosniff", "referrer-policy": "no-referrer" } }); }
export async function handleMiniAppRequestV2(request: Request, env: MiniAppEnv): Promise<Response | null> {
  const url = new URL(request.url);
  if (url.pathname === MINI_APP_PATH || url.pathname === `${MINI_APP_PATH}/`) { if (request.method !== "GET" && request.method !== "HEAD") return new Response("Method not allowed", { status: 405 }); return request.method === "HEAD" ? new Response(null, { headers: htmlResponse().headers }) : htmlResponse(); }

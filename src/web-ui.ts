...
.playerCenter{position:absolute;z-index:5;left:50%;top:50%;transform:translate(-50%,-50%);width:60px;height:60px;border-radius:50%;display:grid;place-items:center;background:rgba(6,6,8,.54);color:#fff;border:1px solid rgba(255,255,255,.14);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);box-shadow:0 10px 36px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.1);transition:opacity .22s,transform .2s,background .2s}
.playerCenter:active{transform:translate(-50%,-50%) scale(.91)}.playerCenter svg{width:28px;height:28px}.playerShell.playing .playerCenter{opacity:0;pointer-events:none}
...
.playerSettings{position:absolute;z-index:10;right:10px;top:10px;width:38px;height:38px;border-radius:12px;display:grid;place-items:center;background:rgba(7,7,9,.38);color:#fff;border:1px solid rgba(255,255,255,.12);-webkit-backdrop-filter:blur(9px);backdrop-filter:blur(9px);box-shadow:inset 0 1px 0 rgba(255,255,255,.09),0 7px 22px rgba(0,0,0,.16);transition:transform .18s,background .18s,border-color .18s}
.playerSettings:active{transform:scale(.92)}.playerSettings svg{width:21px;height:21px;transition:transform .34s cubic-bezier(.16,1,.3,1)}.playerShell.settingsOpen .playerSettings{border-color:rgba(255,255,255,.22);background:rgba(10,10,13,.54)}.playerShell.settingsOpen .playerSettings svg{transform:rotate(45deg)}
...
.playerControls{position:absolute;z-index:7;left:0;right:0;bottom:0;padding:44px 12px 7px;background:linear-gradient(to top,rgba(0,0,0,.88),rgba(0,0,0,.34) 46%,transparent);transition:opacity .2s,transform .2s}
.playerShell.controlsHidden:not(.settingsOpen) .playerControls{opacity:0;transform:translateY(7px);pointer-events:none}
.controlRow{display:flex;align-items:center;gap:1px;margin:0 0 1px}.controlButton{width:37px;height:37px;flex:0 0 auto;border-radius:11px;display:grid;place-items:center;background:transparent;color:#fff;transition:background .16s,transform .16s,opacity .16s}.controlButton:hover{background:rgba(255,255,255,.055)}.controlButton:active{transform:scale(.9);background:rgba(255,255,255,.09)}.controlButton svg{width:21px;height:21px;overflow:visible}.playButton{width:40px;height:40px}.playButton svg{width:23px;height:23px}.iconMorph{transform-origin:center;animation:iconMorphIn .24s cubic-bezier(.16,1,.3,1)}
.skipButton{width:39px;height:39px}.skipGlyph{position:relative;width:25px;height:25px;display:grid;place-items:center}.skipGlyph svg{position:absolute;inset:0;width:25px;height:25px;transition:transform .22s cubic-bezier(.16,1,.3,1)}.skipValue{position:relative;z-index:1;margin-top:1px;font-size:8px;font-weight:800;line-height:1;letter-spacing:-.04em}.backSkip:active .skipGlyph svg{transform:rotate(-18deg)}.forwardSkip:active .skipGlyph svg{transform:rotate(18deg)}
.playerTime{margin-left:5px;margin-right:2px;color:#d8d8d8;font-size:10.5px;font-variant-numeric:tabular-nums;white-space:nowrap}.seekWrap{position:relative;height:17px;display:flex;align-items:flex-end}.seekBase{position:absolute;left:2px;right:2px;bottom:5px;height:3px;border-radius:99px;background:rgba(255,255,255,.17);overflow:hidden}.bufferedBar,.playedBar{position:absolute;inset:0 auto 0 0;width:0;border-radius:99px}.bufferedBar{background:rgba(255,255,255,.22)}.playedBar{background:#fff}.seek{position:relative;width:100%;height:17px;margin:0;appearance:none;-webkit-appearance:none;background:transparent}.seek::-webkit-slider-runnable-track{height:3px;background:transparent}.seek::-webkit-slider-thumb{-webkit-appearance:none;width:12px;height:12px;margin-top:-4.5px;border-radius:50%;background:#fff;box-shadow:0 1px 7px rgba(0,0,0,.42);transition:transform .14s}.seek:active::-webkit-slider-thumb{transform:scale(1.18)}.seek::-moz-range-track{height:3px;background:transparent}.seek::-moz-range-thumb{width:12px;height:12px;border:0;border-radius:50%;background:#fff}
...
.playerShell:fullscreen{border:0;border-radius:0}.playerShell:fullscreen .playerControls{padding-bottom:max(8px,env(safe-area-inset-bottom))}.playerShell:fullscreen .playerSettings{top:max(10px,env(safe-area-inset-top));right:max(10px,env(safe-area-inset-right))}.playerShell:fullscreen .qualityMenu{top:calc(max(10px,env(safe-area-inset-top)) + 44px);right:max(10px,env(safe-area-inset-right))}
...
@media(max-width:29rem){.page{padding-left:13px;padding-right:13px}.inputShell{grid-template-columns:auto minmax(0,1fr);gap:8px}.go{grid-column:1/-1;width:100%}.formats{grid-template-columns:repeat(2,minmax(0,1fr))}.controlButton{width:34px;height:35px}.playButton{width:37px;height:37px}.skipButton{width:36px;height:36px}.playerTime{margin-left:3px;font-size:9.5px}.playerControls{padding-left:8px;padding-right:8px;padding-bottom:5px}.playerSettings{right:8px;top:8px}.qualityMenu{right:8px;top:52px}.watchMeta{padding-top:15px}}
@media(any-pointer:coarse){.modeButton,.go,.primary,.save,.format{min-height:48px}.controlButton{width:42px;height:42px}.playButton{width:44px;height:44px}.skipButton{width:43px;height:43px}.url{min-height:48px}.seekWrap,.seek{height:22px}.seekBase{bottom:7px}}
        ...
        <div id="playerShell" class="playerShell">
          ...
          <div class="playerControls">
            <div class="controlRow">
              <button id="back10" class="controlButton skipButton backSkip" type="button" aria-label="Back 10 seconds"><span class="skipGlyph"><svg viewBox="0 0 28 28" fill="none" aria-hidden="true"><path d="M8.3 8.1H4.8V4.6M5 8.2a10 10 0 1 1-1 7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="skipValue">10</span></span></button>
              ...
              <button id="forward10" class="controlButton skipButton forwardSkip" type="button" aria-label="Forward 10 seconds"><span class="skipGlyph"><svg viewBox="0 0 28 28" fill="none" aria-hidden="true"><path d="M19.7 8.1h3.5V4.6M23 8.2a10 10 0 1 0 1 7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="skipValue">10</span></span></button>
              <div id="playerTime" class="playerTime">0:00 / 0:00</div>
              <button id="mute" class="controlButton" type="button" aria-label="Mute or unmute"><svg id="muteIcon" viewBox="0 0 24 24" fill="none"><g class="iconMorph"><path d="M5 10v4h4l4 4V6L9 10H5Z" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/><path d="M16 9.2a4 4 0 0 1 0 5.6" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/></g></svg></button>
              <button id="fullscreen" class="controlButton" type="button" aria-label="Fullscreen"><svg viewBox="0 0 24 24" fill="none"><path d="M8 4H4v4M16 4h4v4M8 20H4v-4m12 4h4v-4" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
            </div>
...
var layout=$('layout'),workspace=$('workspace'),modeSwitch=$('modeSwitch'),modeDownload=$('modeDownload'),modeWatch=$('modeWatch'),url=$('url'),analyze=$('analyze'),message=$('message'),result=$('result'),videoTitle=$('videoTitle'),formats=$('formats'),prepare=$('prepare'),progress=$('progress'),progressSub=$('progressSub'),ready=$('ready'),readyMeta=$('readyMeta'),save=$('save'),playerCard=$('playerCard'),playerShell=$('playerShell'),video=$('video'),centerPlay=$('centerPlay'),centerPlayIcon=$('centerPlayIcon'),playerSettings=$('playerSettings'),qualityMenu=$('qualityMenu'),playPause=$('playPause'),playIcon=$('playIcon'),back10=$('back10'),forward10=$('forward10'),seek=$('seek'),bufferedBar=$('bufferedBar'),playedBar=$('playedBar'),playerTime=$('playerTime'),mute=$('mute'),muteIcon=$('muteIcon'),fullscreen=$('fullscreen'),watchTitle=$('watchTitle');
...
back10.addEventListener('click',function(event){event.stopPropagation();setSettingsOpen(false);video.currentTime=Math.max(0,(video.currentTime||0)-10);showControls()});forward10.addEventListener('click',function(event){event.stopPropagation();setSettingsOpen(false);video.currentTime=Math.min(video.duration||Infinity,(video.currentTime||0)+10);showControls()});

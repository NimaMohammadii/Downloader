...
def delete_status(token: str, chat_id: int, message_id: int | None) -> None:
    ...
    try:
        ...
        # A stale status message should never make a successful download fail.
        pass
...
def friendly_ytdlp_error(stderr: str) -> str:
    ...
    if "sign in to confirm" in value or "not a bot" in value:
        return "❌ یوتیوب این دانلود رو موقتاً محدود کرده. یک‌بار دیگه امتحان کن."
    if "http error 403" in value or "403: forbidden" in value:
        return "❌ یوتیوب دسترسی به این ویدیو رو موقتاً محدود کرده. بعداً دوباره امتحان کن."
    if "age" in value and ("restricted" in value or "confirm" in value):
    ...
    return "❌ نتونستم این ویدیو رو از YouTube بگیرم. یک لینک دیگه یا دوباره همین لینک رو امتحان کن."
...
def download_video(url: str, workdir: Path) -> Path:
    ...
    media_files = [
        ...
        if path.is_file() and not path.name.startswith("part_") and path.suffix.lower() not in {".part", ".ytdl"}
    ]
    if not media_files:
...
def send_media_file(
    ...
    *,
    ...
    try:
        ...
        # Some MP4 codec combinations are downloadable but Telegram refuses them as
        # playable video. Sending the same file as a document is a safe fallback.
        try:
            with path.open("rb") as handle:
                telegram_call(
                    token,
...
def process_job(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    status_message_id_raw = payload.get("statusMessageId")
    status_message_id = int(status_message_id_raw) if status_message_id_raw else None
    ...
    try:
        edit_status(token, chat_id, status_message_id, "🔎 دارم ویدیو رو بررسی می‌کنم…")
        ...
        if metadata.get("is_live") or live_status in {"is_live", "is_upcoming"}:
            raise UserVisibleError("❌ دانلود Live فعلاً پشتیبانی نمی‌شه. لینک ویدیوی عادی رو بفرست.")

        title = str(metadata.get("title") or "YouTube video").strip()
        edit_status(token, chat_id, status_message_id, "⬇️ دارم با بهترین حالت سریع تا 720p دانلودش می‌کنم…")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path in {"/", "/health", "/ping"}:
            self._send_json(200, {"ok": True, "service": "yt-dlp-container"})
            return
    ...
    def do_POST(self) -> None:
        ...
        if content_length <= 0 or content_length > 128_000:
        ...
        try:
            ...
            self._send_json(500, {"ok": False, "error": "container failure"})
...
if __name__ == "__main__":
    ...
    print(f"yt-dlp container listening on :{PORT}", flush=True)
